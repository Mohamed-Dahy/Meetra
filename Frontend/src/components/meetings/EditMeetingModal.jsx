// components/meetings/EditMeetingModal.jsx
 ...
 * @param {function} props.onSave - Called with { id, title, participants } after save
 * @param {Object} props.meeting - Meeting object to edit
 * @returns {JSX.Element}
 */
export default function EditMeetingModal({ open, onClose, onSave, meeting }) {
  const [title, setTitle] = useState(meeting?.title || "");
  const [participants, setParticipants] = useState([]);
  const [selectedParticipants, setSelectedParticipants] = useState(meeting?.participants || []);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Update state when meeting changes
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await onSave({ id: meeting._id, title, participants: selectedParticipants });
      onClose();
    } catch (err) {
