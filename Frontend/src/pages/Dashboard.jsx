...
import {
  ...
  BarChart2, Users, ArrowUpRight, Pencil, Trash2
} from "lucide-react";
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .stat-card:hover { border-color: var(--border-hi); box-shadow: 0 0 30px rgba(99,102,241,0.08); }
  .stat-card::before {
  ...
  .meeting-row:hover { background: rgba(99,102,241,0.06); border-color: rgba(99,102,241,0.2); }

  .upload-zone {
    border: 2px dashed rgba(99,102,241,0.3);
    border-radius: 16px; padding: 40px 24px;
    text-align: center; cursor: pointer;
    transition: all 0.3s; background: rgba(99,102,241,0.03);
  }
  ...
  .btn-glow {
    ...
    transition: all 0.2s; box-shadow: 0 0 24px rgba(99,102,241,0.3);
    text-decoration: none;
  }
  .btn-glow:hover { transform:translateY(-2px); box-shadow:0 0 40px rgba(99,102,241,0.5); }

  .health-bar { height: 6px; border-radius: 99px; background: rgba(255,255,255,0.06); overflow: hidden; }

  .badge { display:inline-flex; align-items:center; padding:3px 10px; border-radius:99px; font-size:11px; font-weight:600; font-family:var(--sora); }
  ...
  .section-card { background: var(--bg-card); border: 1px solid var(--border); border-radius: 20px; padding: 28px; }
  .section-title {
  ...
  .err-box { background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.2); border-radius:11px; padding:12px 14px; font-size:13px; color:#f87171; margin-top:12px; }

  .icon-btn { background:none; border:none; cursor:pointer; border-radius:6px; padding:5px; display:flex; align-items:center; transition:all 0.2s; }
...
function StatusBadge({ status }) {
  ...
  if (status === "failed") return <span className="badge" style={{background:"rgba(239,68,68,0.12)",color:"#f87171",border:"1px solid rgba(239,68,68,0.2)"}}>Failed</span>;
      ...
      <div className="dash-noise" style={{ background:"var(--bg)", minHeight:"100vh", fontFamily:"var(--dm)" }}>
        ...
        <div className="topbar">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <button className="btn-glow" onClick={() => { setActiveNav("meetings"); setShowCreate(true); }}>
              <Plus size={14}/> New Meeting
            </button>
            <button style={{ background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--muted2)" }}>
              <Bell size={15}/>
            </button>
              ...
              {activeNav === "overview" && (
                <motion.div variants={stagger} initial="hidden" animate="visible">
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:28 }}>
                    {[
                      { label:"Total Meetings",  val: meetingsLoading ? "—" : meetings.length,                                          delta:"all time",       icon:<FileText    size={18}/>, color:"#6366f1" },
                      { label:"Completed",        val: meetingsLoading ? "—" : meetings.filter(m=>m.status==="completed").length,        delta:"analyzed",       icon:<CheckSquare size={18}/>, color:"#8b5cf6" },
                      { label:"Processing",       val: meetingsLoading ? "—" : meetings.filter(m=>m.status==="processing").length,       delta:"in progress",    icon:<TrendingUp  size={18}/>, color:"#06b6d4" },
                      { label:"Pending",          val: meetingsLoading ? "—" : meetings.filter(m=>m.status==="pending").length,          delta:"awaiting upload", icon:<Zap         size={18}/>, color:"#f59e0b" },
                    ].map((s,i) => (
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:20, marginBottom:20 }}>
                    ...
                    <motion.div variants={fadeUp} custom={4} className="section-card">
                      <div className="section-title">
                        Recent Meetings
                        <button style={{ fontSize:12,color:"#818cf8",background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:4 }}
                      ...
                      {!meetingsLoading && meetings.slice(0,4).map((m,i) => (
                        <div key={m._id} className="meeting-row">
                      ...
                      {!meetingsLoading && meetings.length === 0 && (
                        <div style={{ textAlign:"center",padding:"32px 0",color:"var(--muted)" }}>
                          <FileText size={32} style={{ color:"#818cf8",margin:"0 auto 8px" }}/>
                          <div style={{ fontSize:14 }}>No meetings yet</div>
                        </div>
                      )}
                    </motion.div>
                  ...
                  {/* Upload zone */}
                  <motion.div variants={fadeUp} custom={6} className="upload-zone"
              ...
              {activeNav === "meetings" && (
                <motion.div variants={stagger} initial="hidden" animate="visible">
                  <motion.div variants={fadeUp} style={{ marginBottom:24, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    ...
                    <button className="btn-glow" onClick={() => setShowCreate(true)}>
                      <Plus size={14}/> New Meeting
                    </button>
                  </motion.div>
                  ...
                  <motion.div variants={fadeUp} custom={1} className="section-card">
                    <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                      ...
                      {/* Loading skeletons */}
                      {meetingsLoading && Array.from({length:3}).map((_,i) => (
                        <div key={i} className="skeleton" style={{ height:64, marginBottom:4, borderRadius:12 }}/>
                      ))}

                      {/* Meeting rows */}
                      {!meetingsLoading && meetings.length > 0 && meetings.map((m) => (
                        <div key={m._id} className="meeting-row" style={{ padding:"18px 20px" }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontSize:15,fontWeight:600,color:"var(--text)",marginBottom:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{m.title}</div>
                            <div style={{ fontSize:12,color:"var(--muted)",display:"flex",alignItems:"center",gap:8 }}>
                              <Clock size={11}/>{new Date(m.createdAt).toLocaleDateString()}
                            </div>
                          </div>
                          <div style={{ display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
                            ...
                            <button title="Edit" className="icon-btn" style={{ color:"#a1a1aa" }} onClick={() => setEditMeeting(m)}>
                              <Pencil size={15}/>
                            </button>
                            <button title="Delete" className="icon-btn" style={{ color:"#ef4444" }} onClick={() => setDeleteMeeting(m)}>
                              <Trash2 size={15}/>
                            </button>
                          </div>
                        </div>
                      ...
                      {!meetingsLoading && meetings.length === 0 && (
                        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"48px 0" }}>
                          <FileText size={40} style={{ color:"#818cf8",marginBottom:12 }}/>
                          <div style={{ fontFamily:"var(--sora)",fontSize:18,fontWeight:700,color:"#fff",marginBottom:6 }}>No meetings yet</div>
                          <div style={{ fontSize:14,color:"var(--muted)",marginBottom:18 }}>Upload your first meeting to get started</div>
                          <button className="btn-glow" onClick={() => setShowCreate(true)}>
                      ...
                      {/* Error */}
                      {meetingsError && <div className="err-box">{meetingsError}</div>}
                    </div>
                  </motion.div>
                  ...
                  <CreateMeetingModal
                    ...
                    onSuccess={async (title) => { await createMeeting(title); setShowCreate(false); }}
                  />
                  <EditMeetingModal
                    open={!!editMeeting}
                    onClose={() => setEditMeeting(null)}
                    onSave={async (id, title) => { await updateMeeting(id, title); setEditMeeting(null); }}
                    meeting={editMeeting}
                  />
                  <DeleteConfirmModal
                    open={!!deleteMeeting}
                    onClose={() => setDeleteMeeting(null)}
                    onDelete={async (id) => { await deleteMeetingApi(id); setDeleteMeeting(null); }}
